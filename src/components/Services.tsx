...
export default function Services() {
      ...
      <div className="container-professional">
        <div className="text-center mb-12 sm:mb-16">
          <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl font-serif font-bold text-gray-800 mb-4 sm:mb-6">
            Our Premium <span className="text-gradient">Services</span>
          </h2>
          <p className="max-w-xs sm:max-w-md md:max-w-2xl lg:max-w-3xl mx-auto text-sm sm:text-base md:text-lg lg:text-xl text-gray-600 leading-relaxed px-2">
            From intimate gatherings to grand celebrations, we bring your vision to life with exceptional attention to detail and luxury service.
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 lg:gap-8">
                 {services.map((s, i) => (
                   <div key={i} className="card-professional p-4 sm:p-6 lg:p-8 text-center group">
                     <div className="w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 bg-gradient-to-r from-blue-100 to-indigo-100 rounded-2xl sm:rounded-3xl flex items-center justify-center mb-4 sm:mb-6 mx-auto group-hover:scale-110 transition-all duration-500">
                       <div className="w-6 h-6 sm:w-8 sm:h-8 text-primary">
                         {s.icon}
                       </div>
                     </div>
              <h3 className="text-lg sm:text-xl font-serif font-bold text-gray-800 mb-3 sm:mb-4">{s.title}</h3>
              <p className="text-sm sm:text-base text-gray-600 mb-4 sm:mb-6 leading-relaxed">{s.description}</p>
